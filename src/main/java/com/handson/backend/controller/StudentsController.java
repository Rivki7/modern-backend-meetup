...
@RestController
    ...
    @RequestMapping(value = "/lowSat", method = RequestMethod.GET)
    public ResponseEntity<?> getStudentWithSatLessThan(@RequestParam Integer sat)
    {
        return new ResponseEntity<>(studentService.getStudentWithSatLessThan(sat), HttpStatus.OK);
    }
    @Autowired
    EntityManager em;
